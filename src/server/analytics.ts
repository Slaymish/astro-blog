...
/**
 * Where the browser keeps that nonce, in sessionStorage. The booking-ref
 * rewriter mints it and the session beacon reuses the same value as its `s`
 * field, which is what lets a confirmed booking be joined back to a session.
 * Per tab rather than per browser: a booking still joins to the session that
 * made it, without a persistent identifier following the visitor around.
 * Declared here so the client that writes it and the server that validates it
 * cannot drift apart.
 */
export const VISITOR_NONCE_KEY = 'visitor-nonce';

/**
 * The client half of the analytics contract validated by `src/server/analytics.ts`.
 *
 ...
 *
 * It is a module so that `MAX_EVENTS` has exactly one definition, and so the
 * pure parts below can be unit-tested without a DOM.
 */
import { MAX_EVENTS } from '../server/analytics';
...
export function initAnalytics(): void {
  ...
  window.addEventListener(
    'scroll',
    () => {
      if (nextMilestone >= SCROLL_MILESTONES.length) return;

      const scrollable = document.documentElement.scrollHeight - window.innerHeight;
      if (scrollable <= 0) return;

      // Most scroll events cross no milestone. Comparing against the next
      // milestone's pixel position first keeps those off the percentage path.
      const threshold = (SCROLL_MILESTONES[nextMilestone]! / 100) * scrollable;
      if (window.scrollY < threshold) return;

      const percent = scrollDepthPercent(window.scrollY, scrollable);
      if (percent === null) return;
      if (percent > maxScroll) maxScroll = percent;

      while (nextMilestone < SCROLL_MILESTONES.length && percent >= SCROLL_MILESTONES[nextMilestone]!) {
        track('scroll-depth', { depth: SCROLL_MILESTONES[nextMilestone]!, page: pathname });
        nextMilestone += 1;
      }
    },
    { passive: true }

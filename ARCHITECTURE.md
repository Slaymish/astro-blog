...
- Because Bend swaps the scroll container when its canvas activates, the engine listens for scroll in the capture phase rather than binding to a node that may be replaced. Under `prefers-reduced-motion` the pipes and fittings still render — they are structure — but packets, lamps, and the idle heartbeat do not run.

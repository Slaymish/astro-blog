...
- Because Bend swaps the scroll container when its canvas activates, the engine listens for scroll in the capture phase rather than binding to a node that may be replaced. Touch pointers do not reuse desktop hover targets: each primary touch uses its y coordinate as a launch point on the existing left-gutter spine, sending one packet upward and one downward without drawing new pipework. Under `prefers-reduced-motion` the persistent pipes and fittings still render — they are structure — but packets, lamps, the idle heartbeat, and touch pulses do not run.

...
- Because Bend swaps the scroll container when its canvas activates, the engine listens for scroll in the capture phase rather than binding to a node that may be replaced. Touch pointers do not reuse desktop hover targets: each primary touch draws a transient horizontal rail at the contact point and sends one packet toward each viewport edge. Under `prefers-reduced-motion` the persistent pipes and fittings still render — they are structure — but packets, lamps, the idle heartbeat, and touch rails do not run.
